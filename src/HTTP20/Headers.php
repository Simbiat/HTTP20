...
class Headers
{
    ...
    public const array safeMethods = ['GET', 'HEAD', 'POST'];
    ...
    public static function lastModified(int|string $modTime = 0, bool $exit = false): void
    {
        ...
        if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
           $IfModifiedSince = strtotime(mb_substr($_SERVER['HTTP_IF_MODIFIED_SINCE'], 5, encoding: 'UTF-8'));
    ...
    public static function links(array $links = [], string $type = 'header', bool $strictRel = true): string
    {
        ...
        foreach ($links as $link) {
            ...
            if (
                ...
                #Skip the element, since it does not conform with the standard
                continue;
            }
            ...
            if (isset($link['integrity']) && preg_match('/^(sha256|sha384|sha512)-(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=|[A-Za-z0-9+\/]{4})$/', $link['integrity']) === 0) {
