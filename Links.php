...
class Links
{
    ...
    public static function links(array $links = [], #[ExpectedValues(['header', 'head', 'body'])] string $type = 'header', bool $force_cross_origin = false): string
    {
        ...
        foreach ($links as $link) {
            ...
            if (
                ...
                if (
                    $force_cross_origin
                    && !\array_key_exists('crossorigin', $link)
                ) {
    ...
    private static function processTypeProperty(array &$link): bool
    {
        ...
        if (
            ...
            && \preg_match('/'.Common::MIME_REGEX.'/u', $link['type']) !== 1
        ) {
            $link['type'] = '';
        }
        ...
        if (
            \array_key_exists('rel', $link)
            && \preg_match('/((^| )(modulepreload|preload)( |$))+/uir', $link['rel']) === 1
        ) {
            // Force 'as' for stylesheet
            if (
                (
                    !empty($link['type'])
                    && \preg_match('/^text\/css(;.*)?$/uir', $link['type']) === 1
                )
                || (
                    !empty($link['rel'])
            ...
            if (
                (
                    !empty($link['type'])
                    && \preg_match('/^application\/javascript(;.*)?$/uir', $link['type']) === 1
                )
            ) {
                $link['as'] = 'script';
            }
            ...
            if (
                (
                    !empty($link['type'])
                    && \preg_match('/^image\/.*$/uir', $link['type']) === 1
                )
            ) {
                $link['as'] = 'image';
            }
            ...
            if (
                (
                    !empty($link['type'])
                    && \preg_match('/^application\/.*(font|opentype).*$/uir', $link['type']) === 1
                )
            ) {
                $link['as'] = 'font';
            }
            ...
            if (
                (
                    !empty($link['type'])
                    && \preg_match('/^text\/vtt(;.*)?$/uir', $link['type']) === 1
                )
            ) {
                $link['as'] = 'track';
            }
            ...
            if (
                !\array_key_exists('as', $link)
                && \preg_match('/((^| )(modulepreload)( |$))+/uir', $link['rel']) === 1
            ) {
                $link['as'] = 'script';
            }
        }
    ...
    private static function cleanLink(array &$link, #[ExpectedValues(['header', 'head', 'body'])] string $type): bool
    {
        ...
        if (
            \array_key_exists('referrerpolicy', $link)
            && !\in_array(\mb_strtolower($link['referrerpolicy'], 'UTF-8'), self::REFERRER_POLICY, true)
        ) {
        ...
        if (
            \array_key_exists('fetchpriority', $link)
            && !\in_array(\mb_strtolower($link['fetchpriority'], 'UTF-8'), self::FETCH_PRIORITY, true)
        ) {
            $link['fetchpriority'] = 'auto';
        }
        // Remove `hreflang`, if it's a wrong language value
        if (
            \array_key_exists('hreflang', $link)
            && \preg_match(Common::LANGUAGE_TAG_REGEX, $link['hreflang']) !== 1
        ) {
            unset($link['hreflang']);
        }
        // Remove `sizes` if wrong format
        if (
            \array_key_exists('sizes', $link)
            && \preg_match('/((any|[1-9]\d+[xX][1-9]\d+)( |$))+$/uir', $link['sizes']) !== 1
        ) {
            unset($link['sizes']);
        }
        ...
        if (
            \array_key_exists('crossorigin', $link)
            && (
                empty($link['crossorigin'])
                || !\in_array(\mb_strtolower($link['crossorigin'], 'UTF-8'), ['anonymous', 'use-credentials'], true)
            )
        ) {
            $link['crossorigin'] = 'anonymous';
        }
        ...
        if (
            ...
            && (
                $type !== 'header'
                || \preg_match('/'.Common::LANGUAGE_ENC_REGEX.'.*/uir', $link['title*']) !== 1
            )
        ) {
            unset($link['title*']);
        }
        ...
        if (
            \array_key_exists('integrity', $link)
            && !self::processIntegrity($link)
        ) {
            return false;
        }
        // If integrity is set, check that rel type is of proper type, otherwise remove it
        if (
            isset($link['integrity'], $link['rel'])
            && \preg_match('/((^| )(modulepreload|preload|stylesheet)( |$))+/uir', $link['rel']) !== 1
        ) {
    ...
    private static function processIntegrity(array &$link): bool
    {
        if (\preg_match('/^(sha256|sha384|sha512)-(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=|[A-Za-z0-9+\/]{4})$/u', $link['integrity']) === 0) {
            ...
            if (
                \is_array($potential_iri)
                && !empty($potential_iri['host'])
            ) {
            ...
            if (\is_file($link['integrity'])) {
                ...
                if (
                    ...
                    if (Sanitize::whiteString($size)) {
                        if (
                            \array_key_exists('rel', $link)
                            && \preg_match('/((^| )(icon|apple-touch-icon|apple-touch-icon-precomposed)( |$))+/uir', $link['rel']) === 1
                        ) {
                            $link['sizes'] = $size;
                ...
                if (
                    \array_key_exists('type', $link)
                    && \preg_match('/^(application\/javascript|text\/css)$/uir', $link['type']) === 1
                ) {
                    $hash = $link['integrity']
    ...
    private static function isLinkValid(array $link, #[ExpectedValues(['header', 'head', 'body'])] string $type): bool
    {
        ...
        if (
            (
                !\array_key_exists('href', $link)
                && !\array_key_exists('imagesrcset', $link)
            )
            || (
                $type === 'header'
                && !\array_key_exists('href', $link)
        ...
        if (
            (
                !\array_key_exists('rel', $link)
                && !\array_key_exists('itemprop', $link)
            )
            || isset($link['rel'], $link['itemprop'])
            || (
                $type === 'header'
                && !\array_key_exists('rel', $link)
            )
        ...
        if (
            \array_key_exists('rel', $link)
            && !self::isRelValid($link, $type)
        ) {
            return false;
        }
        // Validate `blocking` value
        if (
            isset($link['rel'], $link['blocking'])
            && \mb_strtolower($link['blocking'], 'UTF-8') !== 'render'
        ) {
            return false;
        }
        ...
        if (
            \array_key_exists('imagesrcset', $link)
            && !\array_key_exists('imagesizes', $link)
            && \preg_match('/ \d+w(,|$)/u', $link['imagesrcset']) === 1
        ) {
            return false;
        }
        if (\array_key_exists('as', $link)) {
            ...
            if (
                (\preg_match('/((^| )(preload)( |$))+/uir', $link['rel']) === 1)
                && !\in_array(\mb_strtolower($link['as'], 'UTF-8'), self::AS_VALUES_PRELOAD, true)
            ) {
                return false;
            }
            if (
                (\preg_match('/((^| )(modulepreload)( |$))+/uir', $link['rel']) === 1)
                && !\in_array(\mb_strtolower($link['as'], 'UTF-8'), self::AS_VALUES_MODULEPRELOAD, true)
            ) {
                return false;
            }
        }
    ...
    private static function isRelValid(array $link, #[ExpectedValues(['header', 'head', 'body'])] string $type): bool
    {
        ...
        if (
            $type === 'body'
            && \preg_match(self::ALLOWED_IN_BODY, $link['rel']) !== 1
        ) {
            return false;
        }
        ...
        if (
            ...
            (
                !\array_key_exists('as', $link)
                || $link['as'] !== 'image'
                || \preg_match('/((^| )(preload)( |$))+/iur', $link['rel']) !== 1
            )
        ) {
            return false;
        }
        ...
        if (
            \array_key_exists('sizes', $link)
            && \preg_match('/((^| )(icon|apple-touch-icon|apple-touch-icon-precomposed)( |$))+/iur', $link['rel']) !== 1
        ) {
            return false;
        }
        ...
        if (
            \array_key_exists('as', $link)
            && \preg_match('/((^| )(modulepreload|preload)( |$))+/iur', $link['rel']) !== 1
        ) {
            return false;
        }
